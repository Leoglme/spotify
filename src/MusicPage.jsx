...
class MusicPage extends React.Component{
    ...
    componentDidMount() {
        fetch("http://localhost:8090/spotify/API/?alltrack")
            .then(res => res.json())
            .then(
                (result) => {
                    this.setState({
    ...
    render() {
        ...
        return (<div>
            ...
            {items.slice(page * perPage, page * perPage + perPage).map((item, index) => {
                return(
                    <Card onClick={() => this.handleClick(item.href)} className={classes.root} key={index}>
                        <ListItem button className={classes.colDirection}>
                            <div className={classes.details}>
                                <CardContent className={classes.content}>
                                    ...
                                    <Typography className={classes.textHide} variant="subtitle1" color="textSecondary">
                                        {item.artist}
                                    </Typography>
                                </CardContent>
